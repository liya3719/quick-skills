...
description: "REQ 驱动编码：实现前必读拆解稿(quick-requirement-decomposition)、技术方案(quick-tech-solution)、设计稿 metadata(Figma/MasterGo)、项目规则(AGENTS.md/CLAUDE.md)；写代码前落盘执行计划(每 block 含目标文件/组件/字段/交互/证据)，缺落点 blocked；禁凭截图写 UI；稳定分层 types→api→state→leaf→page→route→tracking；结束后架构对齐报告，关键项 pass 才编译，missing 最多回流 5 轮否则 blocked。Actions: 执行计划, 架构对齐, 分层实现, token JSON, 子方案分任务. Objects: REQ-xxx, 总方案/子方案, Figma/MasterGo metadata, AGENTS.md. Stacks: Vue3, React, ZRN, TS. Triggers: 按拆解开发, 按方案写代码, 执行计划, 架构对齐报告, 禁止截图 UI, 分层实现."
...
IRON LAW：**禁止在拆解、方案、测试用例、设计 metadata、项目规则未覆盖处发明业务规则、接口字段、UI 或交互。** **写代码前须有落盘的《执行计划》，且每个 ready block 具备目标文件、目标组件、数据字段、交互与证据；缺落点只能标 blocked，禁止硬写。** **禁止凭截图/主观目测写 UI**——须 Figma/MasterGo metadata 或已批准的 OPEN。**实现须按稳定层顺序**：types → api → state → leaf → page → route → tracking。**编译验证前须《架构对齐报告》关键项全部 pass**；missing 最多回流 5 轮，仍缺则 **blocked**。
...
- [ ] 步骤 1：必读输入清点 ⛔ BLOCKING
  - [ ] 1.1 **需求拆解**（`quick-requirement-decomposition` 产出）：路径、版本、REQ-xxx 列表
  - [ ] 1.2 **技术方案**（`quick-tech-solution` 产出）：总方案 + 附录 F + 子方案；设计分块可识别
  - [ ] 1.3 **设计稿 metadata**：Figma / MasterGo 可解析文件路径（非截图）
  - [ ] 1.4 **项目规则**：`AGENTS.md` / `AGENT.md` / `CLAUDE.md`（存在则必读）
  - [ ] 1.5 design token JSON 路径（UI 相关 REQ 时）
  - [ ] 1.6 **技术栈**：UI 框架、样式方案、语言（未说明则先问）
  - [ ] 1.7（可选）测试用例路径；TC-xxx
  - [ ] 1.8 OPEN/BLOCKED 与假设
- [ ] 步骤 2：设计稿 metadata 门禁 ⛔ BLOCKING
  - [ ] 加载 `references/design-metadata-gate.md`
  - [ ] UI 相关 REQ 无 metadata → 流程 blocked，禁止写 UI
- [ ] 步骤 3：架构与子任务 ⚠️ REQUIRED（多子方案时）
  - [ ] 加载 `references/architecture-and-task-split.md`；核对 PRD/方案/附录 F
- [ ] 步骤 4：执行计划 ⛔ BLOCKING（真正写代码前）
...
**四类真源缺一不可**（UI 无关迭代可 1.3 标 UI-N/A + OPEN，须用户确认）：

| 真源 | 来源 skill / 路径 |
|------|-------------------|
| 需求 | `quick-requirement-decomposition` → `docs/prd/` 或约定拆解稿 |
| 方案 | `quick-tech-solution` → `docs/design/` 总方案 + 子方案 |
| 设计 metadata | Figma / MasterGo 导出（节点树、token、DSL） |
| 项目规则 | 仓库根 `AGENTS.md` / `AGENT.md` / `CLAUDE.md` |

自问：四类路径是否已 Read？版本是否与 REQ/方案一致？**未 Read 禁止步骤 4 之后任何业务代码编辑。**

## 步骤 2–4：门禁与执行计划

- **步骤 2**：`design-metadata-gate.md` — 无 metadata 则 UI 层 blocked
- **步骤 4**：`execution-plan.md` — 方案每个设计分块 → 一行 block；**无证据不得标 ready**

默认执行计划路径：`docs/ai/codegen/{需求名}-执行计划-v{x.y}.md`

## 步骤 5–6：契约、token、分层顺序

步骤 5 沿用 design token 与栈 reference（见下表）。

步骤 6 **稳定顺序**（`implementation-layer-order.md`）：
...
| metadata 门禁 | `design-metadata-gate.md` |
...
- **quick-tech-solution**：总方案、子方案、附录 F、设计分块与追溯矩阵以方案为准；执行计划 block 须回溯方案章节。
- **quick-requirement-testcase-trace**：实现应对齐 TC 或说明 BLOCKED；对齐报告「待验证项」可引用 TC。
- **quick-visual-audit**：步骤 9 UI 验收；产出 VA 偏差清单，本 skill 按清单回流修复。
- **quick-compile-verify**：步骤 10；视觉 pass 或 UI-N/A 后执行。
